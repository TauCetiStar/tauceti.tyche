 ...
 */
public class Meanof5RangeFilter extends DrawFilterImpl implements DrawFilter {

	protected float min = -1;
	protected float max = -1;

	public Meanof5RangeFilter() {
	}
	 ...
	 * @param max
	 */
	public Meanof5RangeFilter(float min, float max) {
		this.min = min;
		this.max = max;
	}
	...
	public boolean accept(Draw draw) {
		...
		if (this.min <= mean && mean <= this.max) {
			return true;
		}
		return false;
	}
	...
	public String toString() {
		return "Mean-Of-5 [" + this.min + ", " + this.max + "]";
	}
}

 ...
 * 
 */
public class ReportGeneratorChartImpl extends AbstractReportGenerator {

	public static String ID = "ReportGeneratorChartImpl";

	public ReportGeneratorChartImpl() {
		setId(ID);
	}
	...
	public Report generate(Map<String, Object> args) {
		...
		try {
			String fileName = reportId + "_chart.html";
			if (reportId.contains("_predic_")) {
	...
	protected String getHtmlContent(Map<String, Object> args) {
		...
		try {
			...
			if (DrawHelper.INSTANCE.isPowerball(args)) {
				topContent = topContent.replaceAll("Lottery", "Powerball - Chart");
			} else if (DrawHelper.INSTANCE.isMegaMillion(args)) {
				topContent = topContent.replaceAll("Lottery", "MegaMillion - Chart");
			}
		} catch (IOException e) {
			e.printStackTrace();
		} finally {
